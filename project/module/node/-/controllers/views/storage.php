...
class Storage extends \Controller
{
    ...
    {
        $this->jquery('#dev_project_module_node_storage')->replace($this->view());
    }
        ...
        foreach ($nodeInstances as $nodeInstance) {
            ...
            $v->assign('instance', [
                'SELECT_BUTTON' => $this->c('\std\ui button:view', [
                    'path'    => ':selectInstance',
                    'data'    => $requestData,
                    'content' => $nodeInstance . ($s['selected_instance'] == $nodeInstance ? '<----' : '')
                ])
            ]);
        }
    ...
    private function getNodeInstances()
    {
        ...
        $nodeInstances = StorageModel::where('module_namespace', $moduleNamespace)
